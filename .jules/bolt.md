...
**Learning:** Using `not os.listdir(path)` to check if a directory is empty reads the entire directory contents into a list in memory. For directories with thousands of files (like backup staging areas), this causes unnecessary CPU and memory overhead.
**Action:** Always check if a directory is empty by using `with os.scandir(path) as it: is_empty = not any(it)`. This stops at the first entry, providing an O(1) check and significantly improving performance.
