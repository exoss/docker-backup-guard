...
## 2025-03-05 - Avoid list instantiation overhead inside loops
**Learning:** In tight loops (like iterating over all mounts of all containers), doing `item in ['a', 'b']` recreates the list `['a', 'b']` on every iteration. This adds unnecessary memory allocation overhead.
**Action:** Extract invariant lists into module-level variables. If the list is only used for membership checks (`in`), convert it to a `frozenset` for `O(1)` lookups. Alternatively, use an immutable tuple `('a', 'b')` inline instead of a list `['a', 'b']`.
