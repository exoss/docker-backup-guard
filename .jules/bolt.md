...
## 2024-05-25 - Use os.scandir for O(1) empty directory check
**Learning:** Using `not os.listdir(path)` to check if a directory is empty forces Python to read all directory entries into a list simultaneously, causing memory spikes and high latency for directories containing thousands of files.
**Action:** Replace `not os.listdir(path)` with `with os.scandir(path) as it: is_empty = not any(it)` to check if a directory is empty. `os.scandir` returns an iterator and `any` stops at the first entry, making the check O(1) and eliminating memory overhead.
