...
## 2025-05-18 - Avoid URL Parsing in Heartbeat Loop
**Learning:** Parsing the Uptime Kuma push URL on every heartbeat execution using `urllib.parse` introduces unnecessary CPU overhead and reduces overall performance of the scheduling service. Since the URL and its parameters (`status=up`, `msg=System Idle - Waiting for Schedule`) are static as long as the configuration does not change, recalculating them repeatedly is inefficient. A quick performance test showed a massive difference for 100k executions.
**Action:** Extract the Uptime Kuma push URL logic into a setup/initialization function (`_prepare_heartbeat_url`) that modifies the URL only when the schedule is loaded or configuration changes, and pass the pre-calculated URL to the heartbeat executor.
