...
\n## 2026-04-09 - Pre-calculate immutable sets and use tuples instead of lists for short inline membership checks\n**Learning:** Constructing a `list` directly within a `for` loop or `if` statement for membership checks (e.g., `if type in ['bind', 'volume']`) causes Python to re-allocate memory for that list on every single evaluation. Additionally, list lookups are O(N). This causes unnecessary CPU and memory overhead in frequently executed methods.\n**Action:** Extract invariant lists used purely for membership checks into class-level or module-level `frozenset` objects for O(1) lookups and zero allocation overhead during execution. For short inline checks where extraction isn't necessary, use immutable `tuple` literals (e.g., `('bind', 'volume')`) instead of lists.
