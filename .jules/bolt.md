...
## 2025-03-05 - Avoid list allocation for membership checks
**Learning:** Using lists for invariant collections or inline membership checks (e.g., `x in ['a', 'b']`) incurs unnecessary memory allocation overhead on every evaluation.
**Action:** Extract invariant lists into module-level `frozenset` objects for O(1) lookups, or use tuples (e.g., `x in ('a', 'b')`) which avoid list creation overhead and are faster.
