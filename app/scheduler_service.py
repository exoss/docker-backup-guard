...
def _prepare_heartbeat_url(url):
    """
    Pre-calculates the heartbeat URL, particularly for Uptime Kuma,
    to avoid repeated parsing in the recurring job.
    """
    if not url:
        return url
...
def send_heartbeat(url):
    ...
    Performance optimization: Expects a pre-calculated URL to avoid parsing overhead.
    """
    try:
        # Send Request (GET)
