...
def _prepare_heartbeat_url(url):
    """Pre-calculates the Uptime Kuma push URL to avoid parsing overhead on every tick."""
    final_url = url
    if "/api/push/" in url:
        try:
        ...
        try:
            ...
            if config_changed:
                ...
                if hb_url and hb_interval > 0:
                    # Performance Optimization: Pre-calculate the URL once
                    final_hb_url = _prepare_heartbeat_url(hb_url)
                    schedule.every(hb_interval).minutes.do(send_heartbeat, url=final_hb_url)
