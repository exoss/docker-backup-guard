...
class BackupEngine:
    ...
    def _run_rclone(self, args, timeout=600):
        if not os.path.exists(self.rclone_config):
            self._log(
                f"Rclone configuration file not found: {self.rclone_config}", "WARNING"
            )
            return None
    ...
    def _update_state_file(self, status, size_bytes=0, protected_count=0):
        ...
        try:
            # Performance optimization: Remove LBYL `os.path.exists()` check.
            # EAFP relies on `open()` to throw FileNotFoundError, saving an extra `stat` call.
        ...
        for mount in container.attrs["Mounts"]:
            ...
            if mount["Type"] in VALID_MOUNT_TYPES:
                ...
                if source in EXCLUDED_PATHS:
                    self._log(
                        f"Skipping system path: {source} (Container: {container.name})",
                        "WARNING",
                    )
                    continue
                ...
                if source.startswith(("/proc/", "/sys/", "/dev/", "/run/")):
                    self._log(
                        f"Skipping system sub-path: {source} (Container: {container.name})",
                        "WARNING",
                    )
                    continue
                ...
                if source == "/":
                    self._log(
                        f"Skipping Root FS mount: {source} (Container: {container.name})",
                        "WARNING",
                    )
                    continue
                # -----------------------
    ...
    def get_backup_candidates(self):
        ...
        for container in self.client.containers.list(
            filters={"label": "backup.enable=true"}
        ):
            # Check if it is Portainer
            if self._is_portainer(container):
                if api_configured:
                    self._log(
                        f"Detected Portainer container: {container.name}. Skipping file-level backup in favor of API backup.",
                        "INFO",
                    )
                    continue
                else:
                    self._log(
                        f"Detected Portainer container: {container.name}, but API credentials missing. Falling back to Stop/Copy backup.",
                        "WARNING",
                    )
    ...
    def _cleanup_remote_backups(self, retention_days):
        ...
        target_path = self._get_rclone_target_path()
        self._log(
            f"Running remote cleanup (Retention: {retention_days} days) on {target_path}..."
        )
    ...
    def perform_portainer_backup(self):
        ...
        try:
            ...
            if success:
                self._log(
                    f"Portainer Backup successful and uploaded: {master_archive_name}"
                )
    ...
    def _retry_operation(self, func, retries=3, delay=5, *args, **kwargs):
        ...
        for i in range(retries):
            try:
                ...
                last_exception = e
                self._log(
                    f"Operation failed (Attempt {i+1}/{retries}): {e}. Retrying in {delay}s...",
                    "WARNING",
                )
                time.sleep(delay)
        raise last_exception

    def _process_group_backup(
        self,
        group_name,
        containers,
        backup_tree_root,
        ...
        """
        msg = get_text(lang, "progress_processing_group").format(
            group=group_name, count=len(containers)
        )
        self._log(msg)
        ...
        for container in containers:
            try:
                ...
                # Optimization: O(1) identity check is faster than a dictionary lookup
                if fresh_c is container:
                    fresh_c.reload()

                # Check if container is in a transition state (restarting, paused)
                if fresh_c.status in TRANSITION_STATES:
                    self._log(
                        f"Container {fresh_c.name} is in '{fresh_c.status}' state. Waiting 10s...",
                        "WARNING",
                    )
                    time.sleep(10)
                    fresh_c.reload()
        ...
        try:
            ...
            if progress_callback:
                progress_callback(
                    get_text(lang, "progress_stopping").format(group=group_name)
                )
            ...
            if containers:
                ...
                def _stop_container(container):
                    try:
                        ...
                        self._log(
                            f"Failed to stop {container.name} after retries: {e}",
                            "WARNING",
                        )
                ...
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=min(len(containers), 10)
                ) as executor:
            ...
            for src in unique_paths:
                try:
                    ...
                    if progress_callback:
                        progress_callback(
                            get_text(lang, "progress_snapshot").format(
                                path=relative_path
                            )
                        )
            ...
            if progress_callback:
                progress_callback(
                    get_text(lang, "progress_error_group").format(
                        group=group_name, error=e
                    )
                )
            ...
            if progress_callback:
                progress_callback(
                    get_text(lang, "progress_restarting").format(group=group_name)
                )
            ...
            if stopped_containers:
                ...
                def _start_container(container):
                    try:
                        ...
                        self._log(
                            f"Failed to start {container.name} after retries: {e}",
                            "ERROR",
                        )

                # Performance Optimization: Parallelize container starting to reduce service downtime
                with concurrent.futures.ThreadPoolExecutor(
                    max_workers=min(len(stopped_containers), 10)
                ) as executor:
    ...
    def perform_backup(self, container_id=None, progress_callback=None, lang="en"):
        ...
        try:
            ...
            if result_master.returncode != 0:
                ...
                if progress_callback:
                    progress_callback(
                        get_text(lang, "progress_compression_failed").format(
                            error=result_master.stderr
                        )
                    )
                return False
            ...
            if success:
                ...
                self._log(
                    f"Cloud sync successful. Deleting local archive: {master_archive_name}"
                )
                if progress_callback:
