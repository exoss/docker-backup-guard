...
class BackupEngine:
    # Performance optimization: extract invariant lists into class-level frozenset
    # for O(1) membership lookups, eliminating array allocation overhead.
    ...
    def get_container_volumes(self, container):
        ...
        for mount in container.attrs['Mounts']:
            ...
            # Performance optimization: use immutable tuple for membership check
            if mount['Type'] in ('bind', 'volume'):
                source = mount['Source']
                
                # --- EXCLUSION LOGIC ---
                # 1. Exact match exclusion against O(1) frozenset
